...
public class CommandProcessor {
    ...
    // Исправленный метод setDnd без использования констант, только числовые значения
    private String setDnd(boolean enable) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            return "Режим не поддерживается на этой версии Android";
        }
        try {
            // Используем строку "zen_mode" как ключ и числовые значения:
            // 0 = OFF, 1 = IMPORTANT_INTERRUPTIONS
            Settings.Global.putInt(context.getContentResolver(), "zen_mode", enable ? 1 : 0);
            return enable ? "Режим не беспокоить включён" : "Режим не беспокоить выключен";
        } catch (Exception e) {
            Log.e(TAG, "DND error", e);
            return "Не удалось изменить режим не беспокоить";
        }
    }
